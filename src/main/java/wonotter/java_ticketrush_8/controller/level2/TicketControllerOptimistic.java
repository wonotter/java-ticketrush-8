...
import wonotter.java_ticketrush_8.service.level2.TicketOrderFacade;
import wonotter.java_ticketrush_8.service.level2.TicketServiceOptimistic;

@RestController
@RequestMapping("/api/level2/optimistic/tickets/")
@RequiredArgsConstructor
public class TicketControllerOptimistic {
    ...
    public ResponseEntity<TicketOrderResponse> orderTicket(
        ...
        try {
            Long orderId = ticketOrderFacade.orderTicketWithRetry(ticketId, request.userId());
    ...
    public ResponseEntity<Long> getOrderCount(@PathVariable Long ticketId) {
        long count = ticketServiceOptimistic.getOrderCount(ticketId);
