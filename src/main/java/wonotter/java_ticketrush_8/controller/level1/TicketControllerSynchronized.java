...
import wonotter.java_ticketrush_8.service.level1.TicketServiceSynchronized;

@RestController
@RequestMapping("/api/level1/sync/tickets/")
@RequiredArgsConstructor
public class TicketControllerSynchronized {

    private final TicketServiceSynchronized ticketServiceSynchronized;
    ...
    public ResponseEntity<TicketOrderResponse> orderTicket(
        ...
        try {
            Long orderId = ticketServiceSynchronized.orderTicket(ticketId, request.userId());
    ...
    public ResponseEntity<Long> getOrderCount(@PathVariable Long ticketId) {
        long count = ticketServiceSynchronized.getOrderCount(ticketId);
